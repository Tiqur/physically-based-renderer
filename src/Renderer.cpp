...
void Renderer::setupRasterUniforms(const glm::mat4& model, const glm::mat4& view, 
...
void Renderer::renderRays(const std::vector<Ray*>& rays, const std::vector<Shape*>& worldObjects) {
    if (rays.empty() || rayVAOs.empty()) return;
    ...
    for (size_t i = 0; i < rays.size(); i++) {
        ...
        glm::vec4 color = hitAnything ? 
            glm::vec4(0.0f, 1.0f, 0.0f, 0.05f) : 
            glm::vec4(1.0f, 1.0f, 1.0f, 0.02f);
...
void Renderer::renderShapes(const std::vector<Shape*>& shapes) {
    if (shapes.empty() || shapeVAOs.empty()) return;
...
void Renderer::cleanupShapes() {
    for (VBO* vbo : shapeVBOs) delete vbo;
    for (VAO* vao : shapeVAOs) delete vao;
    shapeVBOs.clear();
    shapeVAOs.clear();
}
...
void Renderer::scrollCallback(GLFWwindow* window, double xoffset, double yoffset) {
    ...
    ImGuiIO& io = ImGui::GetIO();
    if (io.WantCaptureMouse || !instance) return;
...
void Renderer::cursorPositionCallback(GLFWwindow* window, double xpos, double ypos) {
    ...
    ImGuiIO& io = ImGui::GetIO();
    bool shouldDrag = glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS 
                      && !io.WantCaptureMouse;
    ...
    if (shouldDrag) {
        if (!instance->isDragging) {
            ...
            if (instance->cam.getCamPitch() > 89.0f) instance->cam.setCamPitch(89.0f);
            if (instance->cam.getCamPitch() < -89.0f) instance->cam.setCamPitch(-89.0f);
        }
    } else {
